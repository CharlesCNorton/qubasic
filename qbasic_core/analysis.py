...
class AnalysisMixin:
    ...
    def cmd_expect(self, rest: str) -> None:
        ...
        parts = rest.split()
        if not parts:
            print("?USAGE: EXPECT <Z|X|Y|ZZ|...> [qubits]")
            return
        ...
        try:
            ...
            print(f"  <{pauli_str}> on qubits {qubits} = {val.real:.6f}")
    ...
    def cmd_entropy(self, rest: str = '') -> None:
        ...
        try:
            ...
            print(f"  Entanglement entropy S(A) = {ent:.6f} bits")
            print(f"  Partition A: qubits {partition_a}")
            print(f"  Partition B: qubits {[q for q in range(n) if q not in partition_a]}")
            if ent < 0.01:
                print(f"  -> Qubits are separable (product state)")
    ...
    def cmd_density(self) -> None:
        ...
        dim = 2**n
        if dim > 16:
            print(f"  Density matrix: {dim}x{dim} (too large to display)")
            print(f"  Purity: {np.real(np.trace(rho @ rho)):.6f}")
            print(f"  Von Neumann entropy: ", end='')
    ...
    def cmd_bench(self) -> None:
        ...
        print(f"  {'qubits':>8}  {'method':>20}  {'time':>8}{header_mem}")
        for n in [4, 8, 12, 16, 20, 24, 28]:
            ...
            try:
                ...
                print(f"  {n:>8}  {method:>20}  {dt:>7.2f}s{mem_str}")
            except Exception as e:
                print(f"  {n:>8}  {method:>20}  FAILED: {e}")
        print()
    ...
    def cmd_ram(self) -> None:
        ...
        ram = _get_ram_gb()
        if not ram:
            print("?psutil not installed — RAM detection unavailable")
        ...
        print(f"  80% budget: {budget:.1f} GB\n")
        ...
        if self.locc_mode and self.locc:
            ...
            print(f"  Current: LOCC {mode} {sizes_str}q")
            print(f"    Per instance: {est:.2f} GB")
            if max_par > 0:
                print(f"    Parallel:     ~{max_par} instances in budget")
        else:
            n = self.num_qubits
            est = _estimate_gb(n)
            max_par = int(budget / est) if est > 0 else 0
            print(f"  Current: {n} qubits")
            print(f"    Per instance: {est:.2f} GB")
            if max_par > 0:
                print(f"    Parallel:     ~{max_par} instances in budget")
        ...
        for nq in [16, 20, 24, 28, 30, 32]:
            ...
            if mp > 0:
                print(f"    {nq:>2} qubits: {e:>8.2f} GB  ->  ~{mp:>6,} parallel{marker}")
            else:
                print(f"    {nq:>2} qubits: {e:>8.2f} GB  ->  exceeds budget{marker}")
        print()
